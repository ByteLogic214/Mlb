...
# 3. Actualizar ev_calculator.py para soportar múltiples market_types
ev_calculator = '''"""
...
class EVCalculator:
    ...
    def evaluate_batch(self, predictions_df: pd.DataFrame, 
        ...
        Soporta múltiples market_types por juego.
        """
        all_evals = []
        ...
        for _, pred in predictions_df.iterrows():
            ...
            # Buscar cuotas para este juego en todos los mercados permitidos
            odds_match = odds_df[
            ...
            # Evaluar cada market_type disponible
            for _, odds_row in odds_match.iterrows():
                ...
                try:
                    if market_category in ['moneyline', 'run_line']:
                        # Para run_line, necesitamos ajustar la probabilidad por el handicap
                        # Por simplicidad, usamos la misma prob que moneyline
                        # En producción, esto requiere un modelo separado para run lines
                        ...
                        # Para totals, necesitamos probabilidad de over
                        # Por simplicidad, asumimos 50% (neutral) si no hay modelo
                        # En producción, esto requiere un modelo de totals
                        
                        prob_over_point = 0.5  # Placeholder - reemplazar con modelo real
                        prob_over_lower = max(0.5 - 0.15, 0.0)  # Placeholder
                        prob_over_upper = min(0.5 + 0.15, 1.0)  # Placeholder
...
with open(os.path.join(base_dir, "src", "evaluation", "ev_calculator.py"), "w") as f:
    f.write(ev_calculator)

print("✅ ev_calculator.py actualizado para múltiples market_types")
